...
module.exports.friendsUpdateOne = function (req, res) {
    ...
    Friend.findById(friendId).select("-status -interest").exec(function (err, friend) {
        ...
        if (response.status !== 204) {
            ...
            friend.minFriends = parseInt(req.body.minPlayers);
            friend.maxFriends= parseInt(req.body.maxPlayers);
            friend.rate = parseFloat(req.body.rate);
            friend.minAge = parseInt(req.body.minAge);
            friend.interest = {};
...
module.exports.friendsAddOne = function (req, res) {
    Friend.create({
        ...
        birthdate:parseInt(req.body.price),
        minFriends:parseInt(req.body.minPlayers),
        maxFriends:parseInt(req.body.maxPlayers),
